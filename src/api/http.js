...
import {dummyProductAndCustomerData} from "../components/sales/utils";
...
export class GrowthAPI {
    ...
    * initialize the api with authentication token, and an optional business id*/
    ...
    async createSalesOrder({data}, errorHandler){
        return await this._makeRequest({
            url: `/business/${this.business}/sales-orders/`,
            'method': 'POST',
            errorHandler: errorHandler,
            data: data
        })
    ...
    async getProductsAndCustomers(errorHandler){
        return await {
            success: true,
            payload: dummyProductAndCustomerData,
            statucCode: 200
        }
    }
